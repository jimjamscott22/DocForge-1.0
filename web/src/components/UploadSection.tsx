...
type UploadSectionProps = {
  /** When true, omit the outer card chrome (for drawers/modals). */
  embedded?: boolean;
};

export default function UploadSection({ embedded = false }: UploadSectionProps) {
  const [activeTab, setActiveTab] = useState<Tab>("upload");

  const body = (
    <>
      <div className={embedded ? "mb-5" : "mb-5 flex items-center gap-3"}>
        {!embedded && (
